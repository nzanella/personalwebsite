 ...
 */
  ...
  function setRootNodeToDestinationNodePathValue($treeInstance, $rootNodeToDestinationNodeNames /* root name at depth 0 is always 0 */, $destinationNodeValue) {

    // iterate through root node to destination node names array of keys

    $nonTerminalNodeNameCount = count($rootNodeToDestinationNodeNames) - 1;

    for ($depthIndex = 0; $depthIndex < $nonTerminalNodeNameCount; $depthIndex++) {

      $nodeName = $rootNodeToDestinationNodeNames[$depthIndex];

      $treeInstance = $treeInstance[$nodeName];

    }

    $treeInstance[$nonTerminalNodeNameCount] = $destinationNodeValue;
  ...
  function deepModifiedCopy(&$treeInstance, $leafNodeNameTransformationFunction, $leafNodeValueTransformationFunction) {
    ...
    $previousLevelNonLeafNodes = array(/* 0 => */ &$treeInstance); // $pLNLN = array($NodeName1 => $nodeValue1, $nodeValue2, $nodeValue3, ...);
                                                                   // we will have count($pLNLN[$nodeName1]), count($pLNLN[nodeName2]), count($pLNLN[nodeName3]), ...

    // initialize previous level output array of non-leaf nodes to array containing childless output array

    $outputPreviousLevelNonLeafNodes = array(&$outputTreeInstance);
    ...
    while (count($previousLevelNonLeafNodes) > 0) {
      ...
      for ($previousLevelNonLeafNodeIndex = 0; $previousLevelNonLeafNodeIndex < count($previousLevelNonLeafNodes); $previousLevelNonLeafNodeIndex++) {
        ...
        $previousLevelNonLeafNode = &$previousLevelNonLeafNode[$previousLevelNonLeafNodeIndex];

        $outputPreviousLevelNonLeafNode = &$outputPreviousLevelNonLeafNode[$previousLevelNonLeafNodeIndex];
